...
class EncounterMessageManager {
    ...
    func fetchBatchTempIdsFromFirebase(onComplete: ((Error?, ([[String: Any]], Date)?) -> Void)?) {
        Logger.DLog("Fetching Batch of tempIds from API")
        
        let urlString = "\(String.Api.BASE_URL)\(String.Api.GET_TEMPIDS)"
        if let url = URL(string: urlString), let token = AppSettings.userToken, let session = (UIApplication.shared.delegate as? AppDelegate)?.trustKitCertificatePinning?.session {
                    ...
                    if let data = data {
                        do {
                            guard let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String : Any],
                                let tempIdsInBase64 = json["tempIDs"] as? [[String: Any]],
                                let bmRefreshTime = json["refreshTime"] as? Double else {
                                onComplete?(NSError(domain: "BM", code: 9999, userInfo: nil), nil)
                                return
                            }
...
//        let bmRefreshTime = Date().addingTimeInterval(3600 * 12)
//        let tempId: [String: Any] = [
//            "tempID"        :   "tempID1",
//            "startTime"     :   Date(),
//            "expiryTime"    :   bmRefreshTime
//        ]
//        let tempIdsInBase64 = [tempId]
//
//        onComplete?(nil, (tempIdsInBase64, bmRefreshTime))
