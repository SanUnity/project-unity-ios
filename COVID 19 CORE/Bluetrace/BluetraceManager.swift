...
class BluetraceManager {
    ...
    func presentBluetoothAlert(_ bluetoothStateString: String) {
        #if DEBUG
        let alert = UIAlertController(title: "Problema de Bluetooth: "+bluetoothStateString+" en "+DeviceInfo.getModel()+" iOS: "+UIDevice.current.systemVersion, message: "¡Captura de pantalla este mensaje y envíalo a soporte!", preferredStyle: .alert)

        alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: nil))

        DispatchQueue.main.async {
            var topController: UIViewController? = (UIApplication.shared.delegate as? AppDelegate)?.window?.rootViewController
        ...
        #if RELEASE
        let alert = UIAlertController(title: String.Bluetooth.ALERT_TITLE, message: String.Bluetooth.ALERT_MESSAGE, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: String.General.ACCEPT, style: .default, handler: { (_) in
            exit(0)
        }))
        DispatchQueue.main.async {
    ...
    func isBluetoothAuthorized() -> Bool {
        ...
        if #available(iOS 13.1, *) {
            ...
            print(CBPeripheralManager.authorizationStatus())
