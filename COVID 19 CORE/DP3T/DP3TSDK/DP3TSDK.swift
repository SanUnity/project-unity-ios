...
class DP3TSDK {
    ...
    func sync(callback: ((Result<Void, DP3TTracingError>) -> Void)?) {
        ...
        logs.append("\(date) - Contacts in DB: \(state.numberOfContacts), Handsahkes in DB: \(state.numberOfHandshakes)")
...
extension DP3TSDK: DP3TMatcherDelegate {
    func didFindMatch() {
        ...
        switch state.infectionStatus {
        case .exposed:
            logs.append("\(date) - user exposed")
        case .infected:
            logs.append("\(date) - user infected")
        default:
            break
        }
        AppSettings.logs = logs
    ...
    func handShakeAdded(_ handshake: HandshakeModel) {
        if let newHandshaked = try? database.handshakesStorage.count() {
            Analytics.logEvent("token_discovered", parameters: nil)
            AppSettings.exposedLength = AppSettings.exposedLength + 1
            state.numberOfHandshakes = newHandshaked
            if let ephId = String(data: handshake.ephID, encoding: .utf8) {
                let date = "\(Date().timeIntervalSince1970 * 1000)"
                var logs = AppSettings.logs
                logs.append("\(date) - Read token: \(ephId)")
                AppSettings.logs = logs
                Analytics.logEvent("token_readed", parameters: nil)
            }
        }
        #if CALIBRATION
