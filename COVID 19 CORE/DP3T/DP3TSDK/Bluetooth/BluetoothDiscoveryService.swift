...
class BluetoothDiscoveryService: NSObject {
    ...
    private func updateServices() {
        ...
        timer = Timer.scheduledTimer(timeInterval: Default.shared.parameters.contactMatching.windowDuration, target: self, selector: #selector(self.incrementWindow), userInfo: nil, repeats: true)
        timer?.fire()
    ...
    @objc func incrementWindow() {
        Logger.DLog("Increment window")

        AppSettings.windowLength = AppSettings.windowLength + 1
        let date = "\(Date().timeIntervalSince1970 * 1000)"
        var logs = AppSettings.logs
        logs.append("\(date) - New exposure window")
